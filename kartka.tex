...
\usepackage[a4paper,margin=1.1cm, top=1cm,bottom=1cm]{geometry}
...
\changefontsizes[10pt]{8pt} %? stackexchange: how to change latex less 10pt
...
Zatem $fl(x \diamond y) = (x \diamond y)(1 + \delta) $. Jeżeli ($t$ bity mantysy) $x = m_{1}\cdot2^{c_{1}}$ $y = m_{2}\cdot2^{c_{2}}$ oraz $c_{1} -c_{2} > t$ to $fl(x+y) = x$
...
Sama numeryczna poprawność nie gwarantuje dokładności (zależy od cond i K)
\section{Interpolacja}
$f(x)-p_n(x)=\frac{1}{n+1!}f^{(n+1)}(\xi)(x-x_0)\ldots(x-x_n)$
\subsection{Lagrange}
...
Rozkład LU istnieje $\Leftrightarrow$ GE jest wykonalne
...
Jeśli dla macierzy symetrycznej rozkład Ba-Ch nie istnieje to macierz nie jest dodatnio określona.
...
Tw. Jeśli $||B|| < 1$ gdzie $||\cdot||$ jest normą zgodną z pewną normą wektorową to metoda $x^{(k+1)}=Bx^k+c$ jest zbieżna globalnie $|\lambda |\leq ||A||$ dd ze zgodności norm\\
Tw. Gerszgorina niech $A \in \mathbb{C}^{n \times n}$. Dla każdej wartości własnej $\lambda \in \sigma(A)$ istnieje $i \in \{1,\ldots,n\}$ taki, że
$$\lambda \in K_i = \{z \in \mathbb{C}:|z-a_{ii}| \leq \sum_{j=1,j \neq i}^{n} |a_{ij}|$$
gdzie $K_i$ to $i$-te koło Gerszgorina. Ponadto\\
$\sigma(A) \subset \bigcup_{i=1}^nK_i = G(A)$ gdzie $G(A)$ nazywany jest zbiorem Gerszgorina. ($A=A^T \rightarrow \lambda \in \mathbb{R}$)\\
Tw. Jeśłi k kół Gerszgorina tworzy zbiór spójny i rozłączny z pozostałymi kołami to w ytm zbiorze znajduje się dokładnie k wartości własnych
\textbf{Warunki stopu:} \\ %enumerate za dużo miejsca nie chce mi się z nim bawić żeby mniej 
...
$B = -D^{-1}(L+U)$\\
W metodzie Jacobiego warunek dostateczny zbieżności $||B||_{\infty} < 1$ jest spełniony wtedy, gdy macierz $A$ jest silnie diagonalnie dominująca $\left(\sum_{j=1,i\neq j}^n |a_{ij}| < |a_{ii}|\right)$
\begin{algorithmic}
\FOR{$p=1,\ldots,n$}
...
Jeśli A jest silnie diagonalnie dominująca to metoda Gaussa-Seidela jest zbieżna. (na wykładzie te tw. tylko dla Jacobiego)$^?$
...
s\subsection{Warunek zbieżności}
$x = Bx + c$\\
...
Warunki stopu te same oprócz \textbf{4)} $|f(x_k)| \leq d$ (stosować ostrożnie)
\subsection{Metoda Newtona}
$x_{k+1}=x_{k}-\frac{f(x_k)}{f^{\prime}(x_k)}$\\
Jeżeli \textbf{1)} $f$ jest klasy $C^2([a,b])$ \textbf{2)} $f(a)f(b)<0$  \textbf{3)} $f^{\prime}$ i $f^{\prime \prime}$ nie zmieniają znaku na $[a,b]$ \textbf{4)} $x_0$ jest takie, że $f(x_0)f^{\prime \prime}(x_0) > 0$ to metoda newtona jest zbieżna\\
Błąd k-tego przybliżenia można oszacować nierówności z podsekcji \textbf{9.6}\\
Metoda Newtona jest metodą o zbieżności kwadratowej (rząd zbieżności wynosi 2). Wyjątkiem są zera wielokrotne, dla których zbieżność jest liniowa i wynosi 1.
...
\textbf{(Newton)} $x^{(k+1)} = x^{(k)}- [F^\prime(x^{(k)})]^{-1}F(x^{(k)})$ gdzie $F'(x)$ to macierz Jacobiego funkcji $F(x)$ powstałej z układu równań.\\
Oznaczamy $p^{(k)} = x^{(k+1)} -x^{(k)}$ więc
$p^{(k)} = - [F^\prime(x^{(k)})]^{-1}F(x^{(k)})$\\ 
1) obliczamy $F(x^{(k)})$ i $F^\prime(x^{(k)})$\\
2) rozwiązujemy u.r.l. $F^\prime(x^{(k)})p^{(k)} = -F(x^{(k)})$\\
3) Obliczamy $x^{(k+1)} = x^{(k)} + p^{(k)}$
\section{Uwarunkowanie zadania roz. u.r.l i algebra}
\begin{center}
$cond(A) = ||A|| \ ||A^{-1}||$\\ $cond_2(A) = ||A||_2 \ ||A^{-1}||_2 = \left|\frac{\lambda_{max}(A)}{\lambda_{min}(A)}\right|$ dla $A=A^T$\\
Jeśli $A=A^T$, to $||A||_2 = \sqrt{\rho(A^TA)}=\sqrt{\rho(A^2)} =\sqrt{(\rho(A))^2}=\rho(A)$\\
$||A^{-1}||_2 = \left| \frac{1}{\lambda_{min}(A)}\right|$\\
Jeśli $\lambda \in \delta(A)$ oraz $detA \neq 0$ to $\frac{1}{\lambda} \in \delta(A^{-1})$\\
Jeśli $A \in \mathbb{R}^{n\times n}$ oraz $A=A^T$ to $\delta (A) \subset \mathbb{R}$\\
Jeśli $A \in \mathbb{C}^{n\times n}$ oraz $A=A^*$ to $\delta (A) \subset \mathbb{R}$\\
$A^*=\overline{A}^T=\overline{A^T}$ sprzężenie hermitowskie macierzy\\
$A^*A=AA^*=I_n$ macierz unitarna
\end{center}

\section{Węzły Czebyszewa}
$T_n=\cos(n\arccos x)$\\
$T_{k+1} = 2xT_{k}(x)-T_{k-1}(x)$\\
Węzły Czebyszewa na dowolnym przedziale $[a,b]$\\
$$x_i=\frac{a+b}{2}-\frac{b-a}{2}\cos\left(\left(\frac{2i+1}{2n+2}\right)\pi\right)$$
\textbf{Tw.} Fabera Dla dowolnego ciągu układu węzłów istnieje taka funkcja ciągła, że ciąg wielomianów interpolacyjnych, na tych węzłach, nie jest do niej zbieżna\\
\textbf{Tw.} Jeśli $f$ jest funkcją ciągłą to istnieje taki ciąg układów węzłów, że zbudowane na nich wielomiany interpolacyjne tworzą ciąg zbieżny do $f$\\
\textbf{Tw.} Weierstrassa Jeśli funkcja $f$ jest ciągła to dla każdego $\epsilon > 0$ istnieje taki wielomian p, że $\max|f(x)-p(x)| \leq \epsilon$
\end{flushleft}
\thispagestyle{empty}
\end{document}
