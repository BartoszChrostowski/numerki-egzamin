...
Błąd k-tego przybliżenia można oszacować nierówności z subsekcji \textbf{9.6} %(x* to dokładna wartość pierwiastka):\\
%$|x^{*}-x_{k}| \leq \frac{f(x_{k})}{m}$\\
%lub\\
%$|x^{*}-x_{k}| \leq \frac{M}{2m}(x^{*} - x_{k-1})^{2}$\\
%gdzie\\
